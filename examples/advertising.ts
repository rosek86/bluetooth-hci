...
async function createHciAdapter(): Promise<Adapter> {
  ...
  const adapterOptions = getAdapterOptions(args);
  if (!adapterOptions) {
    throw new Error('Invalid input parameters');
  }
...
async function getInputArgs(): Promise<InputArgs|null> {
  const argv = await yargs(hideBin(process.argv)).options({
    'device-type':      { choices: ['serial', 'usb'] , default: 'usb' },
    'usb-vid':          { type: 'number', default: 0x2fe3 },
    'usb-pid':          { type: 'number', default: 0x000d },
...
function getAdapterOptions(args: InputArgs): AdapterParams | null {
