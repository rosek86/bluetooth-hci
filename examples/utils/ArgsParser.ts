...
import { type AdapterParams } from "./HciAdapterFactory.ts";
...
export class ArgsParser {
  public async getInputArgs(defaults?: DefaultInputArgs): Promise<InputArgs | null> {
    ...
    if (
      argv.serialDataBits !== 8 &&
      argv.serialDataBits !== 7 &&
      argv.serialDataBits !== 6 &&
      argv.serialDataBits !== 5
    ) {
      return null;
    }
    if (
      argv.serialParity !== "none" &&
      argv.serialParity !== "even" &&
      argv.serialParity !== "mark" &&
      argv.serialParity !== "odd" &&
