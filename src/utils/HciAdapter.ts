...
import { getCompanyName } from "../../assigned-numbers/Company Identifiers.ts";
import { Hci } from "../hci/Hci.ts";
import { H4 } from "../transport/H4.ts";

import { Address, AddressType } from "./Address.ts";
...
export class HciAdapter extends EventEmitter {
  ...
  public async defaultAdapterSetup(): Promise<void> {
    ...
    console.log(
      ...
      Object.entries(localCommands.Commands)
        .filter(([, value]) => value)
        .map(([key]) => key),
    );
    console.log(
    ...
    console.log(
      "LE Features:",
      Object.entries(leFeatures.Features)
        .filter(([, value]) => value)
        .map(([key]) => key),
    );
